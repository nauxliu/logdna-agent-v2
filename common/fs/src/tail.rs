    ...
    async fn handle_event(
        ...
        match event {
            Event::Initialize(entry_ptr) => {
                ...
                let entry_path = fs.resolve_direct_path(entry, &fs.entries.borrow());

                match entry {
                    Entry::File { name, data, .. } => {
                        // If the file's passes the rules tail it
                        info!(
                            "Initialise event for entry {:?}, target {:?}",
                            name, entry_path
                        );
                        ...
                        if fs.is_initial_dir_target(&entry_path) {
                            return data.borrow_mut().tail(vec![entry_path]).await;
                        }
                    }
                    Entry::Symlink { name, link, .. } => {
                        ...
                        if let Entry::File { data, .. } = &entries.get(entry_key)? {
                            ...
                            if !fs.is_initial_dir_target(link) {
                                return data.tail(vec![entry_path]).await;
                            }
                        }
                    }
                    _ => (),
